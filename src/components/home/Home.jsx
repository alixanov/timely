...
const HomeTask = () => {
  ...
  console.log('HomeTask.jsx - Token:', token);
  ...
  useEffect(() => {
    ...
    if (token) {
      try {
        ...
        console.log('HomeTask.jsx - Decoded user:', user);
        const currentTime = Date.now() / 1000;
        if (user.exp < currentTime) {
          console.log('HomeTask.jsx - Token expired:', user.exp, currentTime);
        ...
        console.error('HomeTask.jsx - Invalid token:', err);
        localStorage.removeItem('token');
      ...
      console.log('HomeTask.jsx - No token found, redirecting to login');
    ...
    const fetchTasks = async () => {
      ...
      if (!userEmail) {
        console.log('HomeTask.jsx - No user email, redirecting to login');
      ...
      setLoading(true);
      try {
        console.log('HomeTask.jsx - Fetching tasks from:', `${API_URL}/tasks?category=Дом`);
        const response = await fetch(`${API_URL}/tasks?category=Дом`, {
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
          },
        });
        console.log('HomeTask.jsx - Fetch response status:', response.status);
        if (!response.ok) {
          const data = await response.json();
          console.log('HomeTask.jsx - Fetch error response:', data);
        ...
        console.error('HomeTask.jsx - Fetch error:', err.message);
  ...
  const handleAddTask = async (e) => {
    e.preventDefault();
    if (!isAuthenticated) {
      console.log('HomeTask.jsx - Not authenticated in handleAddTask, redirecting to login');
      toast.error('Пользователь не авторизован', {
        position: 'top-center',
        autoClose: 3000,
        style: { background: '#1a1a1a', color: '#ffffff', fontFamily: 'Inter, Roboto, sans-serif' },
    ...
    const userEmail = user?.email || '';
    if (!userEmail) {
      console.log('HomeTask.jsx - No user email in handleAddTask, redirecting to login');
      toast.error('Пользователь не авторизован', {
        position: 'top-center',
        autoClose: 3000,
        style: { background: '#1a1a1a', color: '#ffffff', fontFamily: 'Inter, Roboto, sans-serif' },
    ...
    setLoading(true);
    try {
      console.log('HomeTask.jsx - Adding task to:', `${API_URL}/tasks`);
      const response = await fetch(`${API_URL}/tasks`, {
        ...
        body: JSON.stringify({ ...newTask, category: 'Дом', userEmail }),
      });
      console.log('HomeTask.jsx - Add task response status:', response.status);
      if (!response.ok) {
        const data = await response.json();
        console.log('HomeTask.jsx - Add task error response:', data);
        if (response.status === 401) {
          localStorage.removeItem('token');
          setIsAuthenticated(false);
          toast.error('Сессия истекла. Пожалуйста, войдите снова.', {
      ...
      console.error('HomeTask.jsx - Add task error:', err.message);
  ...
  const handleDeleteTask = async (id) => {
    if (!isAuthenticated) {
      console.log('HomeTask.jsx - Not authenticated in handleDeleteTask, redirecting to login');
      toast.error('Пользователь не авторизован', {
        position: 'top-center',
        autoClose: 3000,
        style: { background: '#1a1a1a', color: '#ffffff', fontFamily: 'Inter, Roboto, sans-serif' },
    ...
    const userEmail = user?.email || '';
    if (!userEmail) {
      console.log('HomeTask.jsx - No user email in handleDeleteTask, redirecting to login');
      toast.error('Пользователь не авторизован', {
        position: 'top-center',
        autoClose: 3000,
        style: { background: '#1a1a1a', color: '#ffffff', fontFamily: 'Inter, Roboto, sans-serif' },
    ...
    setLoading(true);
    try {
      console.log('HomeTask.jsx - Deleting task from:', `${API_URL}/tasks/${id}`);
      const response = await fetch(`${API_URL}/tasks/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      console.log('HomeTask.jsx - Delete task response status:', response.status);
      if (!response.ok) {
        const data = await response.json();
        console.log('HomeTask.jsx - Delete task error response:', data);
        if (response.status === 401) {
          localStorage.removeItem('token');
          setIsAuthenticated(false);
          toast.error('Сессия истекла. Пожалуйста, войдите снова.', {
      ...
      console.error('HomeTask.jsx - Delete task error:', err.message);
  ...
  return (
    <HomeContainer>
      <PageTitle variant="h5">Домашние задачи</PageTitle>
      ...
      <TasksContainer>
        {tasks.length === 0 && !loading && (
          <EmptyState>
            <Typography variant="h6">Нет домашних задач</Typography>
            <Typography>Добавьте первую задачу, чтобы начать планирование домашних дел</Typography>
          </EmptyState>
        )}
