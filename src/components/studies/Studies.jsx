...
const StudiesContainer = styled(Box)`
  max-width: 1200px;
  margin: 0 auto;
  padding: 2rem;
  min-height: 100vh;
...
const Studies = () => {
  ...
  console.log('Studies.jsx - Token:', token);
  ...
  useEffect(() => {
    ...
    if (token) {
      try {
        ...
        console.log('Studies.jsx - Decoded user:', user);
        const currentTime = Date.now() / 1000;
        if (user.exp < currentTime) {
          console.log('Studies.jsx - Token expired:', user.exp, currentTime);
        ...
        console.error('Studies.jsx - Invalid token:', err);
        localStorage.removeItem('token');
      ...
      console.log('Studies.jsx - No token found, redirecting to login');
    ...
    const fetchTasks = async () => {
      ...
      if (!userEmail) {
        console.log('Studies.jsx - No user email, redirecting to login');
      ...
      setLoading(true);
      try {
        console.log('Studies.jsx - Fetching tasks from:', `${API_URL}/tasks?category=Учеба`);
        const response = await fetch(`${API_URL}/tasks?category=Учеба`, {
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
          },
        });
        console.log('Studies.jsx - Fetch response status:', response.status);
        if (!response.ok) {
          const data = await response.json();
          console.log('Studies.jsx - Fetch error response:', data);
        ...
        console.error('Studies.jsx - Fetch error:', err.message);
  ...
  const handleAddTask = async (e) => {
    e.preventDefault();
    if (!isAuthenticated) {
      console.log('Studies.jsx - Not authenticated in handleAddTask, redirecting to login');
      toast.error('Пользователь не авторизован', {
        position: 'top-center',
        autoClose: 3000,
        style: { background: '#1a1a1a', color: '#ffffff', fontFamily: 'Inter, Roboto, sans-serif' },
    ...
    const userEmail = user?.email || '';
    if (!userEmail) {
      console.log('Studies.jsx - No user email in handleAddTask, redirecting to login');
      toast.error('Пользователь не авторизован', {
        position: 'top-center',
        autoClose: 3000,
        style: { background: '#1a1a1a', color: '#ffffff', fontFamily: 'Inter, Roboto, sans-serif' },
    ...
    setLoading(true);
    try {
      console.log('Studies.jsx - Adding task to:', `${API_URL}/tasks`);
      const response = await fetch(`${API_URL}/tasks`, {
        ...
        body: JSON.stringify({ ...newTask, category: 'Учеба', userEmail }),
      });
      console.log('Studies.jsx - Add task response status:', response.status);
      if (!response.ok) {
        const data = await response.json();
        console.log('Studies.jsx - Add task error response:', data);
        if (response.status === 401) {
          localStorage.removeItem('token');
          setIsAuthenticated(false);
          toast.error('Сессия истекла. Пожалуйста, войдите снова.', {
      ...
      console.error('Studies.jsx - Add task error:', err.message);
  ...
  const handleDeleteTask = async (id) => {
    if (!isAuthenticated) {
      console.log('Studies.jsx - Not authenticated in handleDeleteTask, redirecting to login');
      toast.error('Пользователь не авторизован', {
        position: 'top-center',
        autoClose: 3000,
        style: { background: '#1a1a1a', color: '#ffffff', fontFamily: 'Inter, Roboto, sans-serif' },
    ...
    const userEmail = user?.email || '';
    if (!userEmail) {
      console.log('Studies.jsx - No user email in handleDeleteTask, redirecting to login');
      toast.error('Пользователь не авторизован', {
        position: 'top-center',
        autoClose: 3000,
        style: { background: '#1a1a1a', color: '#ffffff', fontFamily: 'Inter, Roboto, sans-serif' },
    ...
    setLoading(true);
    try {
      console.log('Studies.jsx - Deleting task from:', `${API_URL}/tasks/${id}`);
      const response = await fetch(`${API_URL}/tasks/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      console.log('Studies.jsx - Delete task response status:', response.status);
      if (!response.ok) {
        const data = await response.json();
        console.log('Studies.jsx - Delete task error response:', data);
        if (response.status === 401) {
          localStorage.removeItem('token');
          setIsAuthenticated(false);
          toast.error('Сессия истекла. Пожалуйста, войдите снова.', {
      ...
      console.error('Studies.jsx - Delete task error:', err.message);
  ...
  return (
    <StudiesContainer>
      <PageTitle variant="h5">Учебные задачи</PageTitle>
      ...
      <TasksContainer>
        {tasks.length === 0 && !loading && (
          <EmptyState>
            <Typography variant="h6">Нет учебных задач</Typography>
            <Typography>Добавьте первую задачу, чтобы начать планирование учебы</Typography>
          </EmptyState>
        )}
